...
Item{
    ...
    Component.onCompleted: {
        searchTableModel.setLineHeight(rowsHeight);
    }
    ...
    TableView {
        ...
        onFlickEnded: {
            var t=searchTableModel.refreshBuffer(dataListView.contentY);
    ...
    QScrollBar {
        ...
        onPositionChanged: {
            if(pressed)
            {
                var t=searchTableModel.scrollPosition(position);
                dataListView.contentY=t;
            }
        }
    }
    ...
    function refreshScrollBar(){
        var count=searchTableModel.getShowCount();
        vbar.position=(searchTableModel.getShowFirst()+dataListView.contentY/rowsHeight)/count;
        vbar.size=Math.min(dataListView.height/rowsHeight/count,1);
    }

    function resetScrollBarPosition(){
        vbar.position=0;
        var t=searchTableModel.scrollPosition(0);
        dataListView.contentY=t;
    }
}
